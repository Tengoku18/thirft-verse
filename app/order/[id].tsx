...
function Row({
  ...
  const content = (
    <View className={`flex-row items-center justify-between px-4 py-3.5 ${!last ? "border-b border-[#F3F4F6]" : ""}`}>
      <View className="flex-row items-center flex-1 mr-3">
        {icon && <IconSymbol name={icon as any} size={16} color="#9CA3AF" style={{ marginRight: 10 }} />}
        <BodyRegularText style={{ color: "#6B7280", fontSize: 14 }}>{label}</BodyRegularText>
      </View>
      <View className="flex-row items-center" style={{ maxWidth: "55%" }}>
        <BodySemiboldText
          style={{ fontSize: 14, color: highlight ? "#3B82F6" : "#1F2937", textAlign: "right" }}
          numberOfLines={2}
        >
          {value}
        </BodySemiboldText>
        {onPress && <IconSymbol name="chevron.right" size={12} color="#D1D5DB" style={{ marginLeft: 6 }} />}
      </View>
    </View>
  );
...
function Timeline({ status, createdAt, updatedAt }: { status: string; createdAt: string; updatedAt: string }) {
  ...
  const steps = [
    { id: "placed", title: "Order Placed", done: true, date: orderDate.format("DD MMM • h:mm A") },
    { id: "confirmed", title: "Confirmed", done: true, date: orderDate.add(1, "minute").format("DD MMM • h:mm A") },
    {
      id: "processing",
      title: "Processing",
      done: status !== "pending",
      current: status === "pending",
      date: status !== "pending" ? orderDate.add(1, "hour").format("DD MMM • h:mm A") : undefined,
    },
    {
      id: "delivered",
      title: "Delivered",
      done: status === "completed",
      date: status === "completed" ? updateDate.format("DD MMM • h:mm A") : undefined,
    },
  ];
        ...
        <View key={step.id} className="flex-row">
          <View className="items-center" style={{ width: 32 }}>
            <View
              ...
              style={{
                backgroundColor: step.done ? "#059669" : step.current ? "#3B82F6" : "#E5E7EB",
              }}
            >
              {step.done ? (
          ...
          <View className="ml-3 pb-4 flex-1">
            <BodySemiboldText
              style={{ fontSize: 13, color: step.done || step.current ? "#1F2937" : "#9CA3AF" }}
            >
              {step.title}
            </BodySemiboldText>
            {step.date && (
...
export default function SingleOrderScreen() {
  ...
  const handleMarkDelivered = () => {
    if (!order || order.type !== "order") {
      Alert.alert("Info", "Status can only be updated for orders with full details.");
      return;
    }
    ...
    return (
      <View className="flex-1 bg-[#FAFAFA]">
        ...
        <View className="flex-1 items-center justify-center">
          ...
          <BodyMediumText style={{ color: "#6B7280", marginTop: 12 }}>Loading...</BodyMediumText>
        </View>
      </View>
    );
  }
      ...
      <View className="flex-1 bg-[#FAFAFA]">
        ...
        <View className="flex-1 items-center justify-center px-8">
          <View className="w-20 h-20 rounded-full bg-[#FEE2E2] items-center justify-center mb-4">
            <IconSymbol name="exclamationmark.triangle.fill" size={36} color="#DC2626" />
          </View>
          <HeadingBoldText style={{ marginBottom: 8 }}>Order Not Found</HeadingBoldText>
  ...
  const canMarkDelivered = isSeller && order.status === "pending" && order.type === "order";
  ...
  return (
    <View className="flex-1 bg-[#FAFAFA]">
      ...
      >
        {/* Status Banner */}
        <View className="mx-4 mt-4 p-4 rounded-2xl" style={{ backgroundColor: statusStyle.bg }}>
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center">
              <IconSymbol name={statusStyle.icon as any} size={22} color={statusStyle.text} />
              <HeadingBoldText style={{ color: statusStyle.text, fontSize: 17, marginLeft: 8 }}>
                {statusStyle.label}
              </HeadingBoldText>
            </View>
            <View className="bg-white/40 px-3 py-1 rounded-full">
              <CaptionText style={{ color: statusStyle.text, fontWeight: "600", fontSize: 11 }}>
                {order.orderCode}
              </CaptionText>
            </View>
          </View>
          {order.type === "product" && (
            <CaptionText style={{ color: statusStyle.text, marginTop: 8, opacity: 0.85 }}>
          ...
          <View className="flex-row p-4">
            ...
            <View className="flex-1 ml-4 justify-center">
              ...
              <View className="flex-row items-center justify-between mt-3">
                ...
                <CaptionText style={{ color: "#6B7280" }}>Qty: {order.quantity}</CaptionText>
              </View>
            </View>
          </View>
          <View className="bg-[#F9FAFB] px-4 py-2.5 flex-row items-center justify-center">
            <CaptionText style={{ color: "#6B7280" }}>Tap to view product</CaptionText>
            <IconSymbol name="chevron.right" size={12} color="#9CA3AF" style={{ marginLeft: 4 }} />
          </View>
        </TouchableOpacity>
        ...
        <Section title="Buyer Information">
          ...
          <Row
            ...
            onPress={order.buyer.email !== "Not available" ? () => handleContact("email") : undefined}
          />
          <Row
            label="Phone"
            value={order.buyer.phone || "Not provided"}
            icon="phone.fill"
            highlight={order.buyer.phone !== "" && order.buyer.phone !== "Not available"}
            onPress={
        ...
        {order.shipping.address && (
          <Section title="Shipping Address">
            <Row label="Method" value={order.shipping.method} icon="shippingbox.fill" />
            ...
            <Row label="Postal Code" value={order.shipping.address.postalCode} />
        ...
        <Section title="Payment Summary">
          <Row label="Method" value={order.payment.method} icon="creditcard.fill" />
          <Row label="Transaction ID" value={order.payment.transactionCode} icon="number" />
          <Row label="Order Date" value={dayjs(order.createdAt).format("DD MMM, YYYY • h:mm A")} icon="calendar" />
          <Row label="Subtotal" value={formatPrice(order.payment.subtotal)} />
          {order.shipping.fee > 0 && <Row label="Shipping" value={formatPrice(order.shipping.fee)} />}
          <View className="px-4 py-4 bg-[#F9FAFB] flex-row items-center justify-between">
            <BodyBoldText style={{ fontSize: 15, color: "#374151" }}>Total</BodyBoldText>
        ...
        <Section title="Order Information">
          <Row label="Order ID" value={order.id.slice(0, 16) + "..."} icon="doc.text.fill" />
          <Row label="Order Code" value={order.orderCode} />
          <Row label="Source" value={order.type === "order" ? "Order Record" : "Sold Product"} />
          <Row label="Last Updated" value={dayjs(order.updatedAt).format("DD MMM, YYYY • h:mm A")} last />
        </Section>
      </ScrollView>
          ...
          >
            {updating ? (
              ...
              <>
                ...
                <BodyBoldText style={{ color: "#FFFFFF", fontSize: 16, marginLeft: 8 }}>
                  Mark as Delivered to Store
                </BodyBoldText>
              </>
            )}
          </TouchableOpacity>
          <CaptionText style={{ color: "#9CA3AF", textAlign: "center", marginTop: 8, fontSize: 12 }}>
